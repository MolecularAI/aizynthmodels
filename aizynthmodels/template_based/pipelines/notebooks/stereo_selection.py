...
def make_stats_by_sources(data, extra_columns=None):
    ...
    for source in sources:
        ...
        source_stats["n reactions with stereo changes in centre"].append(has_changes.sum())
...
print(f"Removing {sel.sum()} reactions where a stereo centre did not change during the reaction")
data = data[~sel]
...
data = data[["RxnSmilesClean", "PseudoHash", "classification", "RingBreaker", "StereoBucket"]]
