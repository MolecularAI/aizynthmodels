...
import pandas as pd
from aizynthtrain.utils.configs import TemplateLibraryColumnsConfig, load_config
...
col_config: TemplateLibraryColumnsConfig = load_config(config_filename, "template_library_columns")
...
print_(f"Number of templates with flipped chirality = {data[sel]['FlippedStereo'].sum()}")
...
print_(f"Number of discarded unique templates = {len(set(discarded_data[col_config.template_hash]))}")
...
nunique = [len(set(data[data[col_config.stereo_bucket] == bucket][col_config.template_hash])) for bucket in buckets]
display(  # noqa: F821
    pd.DataFrame(
        {
            "category": buckets,
