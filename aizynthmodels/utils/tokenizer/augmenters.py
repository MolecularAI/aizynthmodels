...
class SMILESAugmenter(MolAugmenter):
    ...
    def __init__(self, active: bool = True, augment_prob: float = 1.0, restricted: bool = True) -> None:
    ...
    def augment_smiles(self, smiles: Iterable[str]) -> List[str]:
        ...
        for smi in smiles:
            ...
            mols: List[Mol] = list(map(Chem.MolFromSmiles, split_smiles_from_reaction(smi)))
            for _ in range(3):
                try:
                    ...
                    logging.info(f"Augmentation failed for {smi} with error: {e}")
            else:
                smiles_aug.append(smi)
                logging.info(f"Augmentation failed three times for {smi}, returning unaugmented original")
