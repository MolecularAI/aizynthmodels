...
class SMILESTokenizer:
    ...
    def __call__(self, data: Union[str, List[str]], *args, **kwargs) -> List[torch.Tensor]:
    ...
    def tokenize(self, data: List[str], enclose: bool = True) -> List[List[str]]:
        ...
        for smi in data:
            ...
            if enclose:
                tokenized_data.append([self._beginning_of_smiles_token] + tokens + [self._end_of_smiles_token])
            else:
    ...
    def convert_tokens_to_ids(self, token_data: List[List[str]]) -> List[torch.Tensor]:
        ...
        for tokens, length in zip(token_data, tokens_lengths):
            ...
            for tdx, token in enumerate(tokens):
                ids_tensor[tdx] = self.vocabulary.get(token, self.vocabulary[self._unknown_token])
    ...
    def decode(self, encoded_data: List[torch.Tensor], encoding_type: Optional[str] = None) -> List[str]:
    ...
    def add_regex_token_patterns(self, tokens: List[str], smiles: List[str] = None) -> None:
    ...
    def _strip_list(
        ...
        if strip_control_tokens:
            strip_characters.extend([self._beginning_of_smiles_token, self._end_of_smiles_token])
    ...
    def _get_compiled_regex(self, tokens: List[str], regex_tokens: List[str]) -> Pattern:
    ...
    def _check_encoding_type(self, encoding_type: str) -> None:
        ...
        if encoding_type not in {"one hot", "index"}:
            raise ValueError(f"unknown choice of encoding: {encoding_type}, muse be either 'one hot' or 'index'")
    ...
    def _state_properties(self) -> Dict[str, Any]:
        ...
        dict_["special_tokens"] = {name: val for name, val in self.special_tokens.items()}
        return dict_
