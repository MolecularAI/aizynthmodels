"""
Schema definition of the hierarchical config files for template-based model.
"""
...
from aizynthmodels.utils.configs.template_based.filename_postfixes import FilenamePostfixes
    ...
    datamodule: Optional[StrDict] = field(
        default_factory=lambda: {
            ...
            "arguments": [
                ...
                {"inputs_postfix": "${filename_postfixes.model_inputs}"},
                {"labels_postfix": "${filename_postfixes.model_labels}"},
                {"reactions_postfix": "${filename_postfixes.library}"},
            ],
        }
    )
