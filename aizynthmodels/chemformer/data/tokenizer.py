...
class ChemformerTokenizer(SMILESTokenizer):
    ...
    tokenizer_name = "ChemformerTokenizer"
...
def build_masker(config: DictConfig, tokenizer: ChemformerTokenizer) -> Optional[TokensMasker]:
    """Load a masker object from config file."""
    masker_config = config.get("masker")

    if not masker_config or not masker_config["type"]:
        return None

    extra_kwargs = {"tokenizer": tokenizer}
    if masker_config.get("arguments"):
        extra_kwargs.update(unravel_list_dict(masker_config.arguments))

    cls, kwargs, _ = load_item(masker_config.type, __name__, extra_kwargs)
    obj = cls(**kwargs)
    config_str = f" with configuration '{kwargs}'"
    logging.info(f"Loaded masker: '{repr(obj)}'{config_str}")
    return obj
