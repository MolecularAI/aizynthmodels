...
from aizynthmodels.chemformer.data.tokenizer import ChemformerTokenizer, ReplaceTokensMasker, TokensMasker
...
class ChemistryDataset(Dataset):
    ...
    def __init__(self, data: StrDict) -> None:
        self._data = data
        if self._data:
    ...
    def __getitem__(self, item: int) -> StrDict:
...
class BaseDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(f"Using a batch size of {str(batch_size)}.")
        ...
        if self.n_chunks > 1:
            logging.info("Using chunk of data:")
            logging.info(f"- i_chunk: {i_chunk}, n_chunks: {n_chunks}")

        self._all_data: StrDict = {}
    ...
    def full_dataloader(self, train=False):
        ...
        if self.n_chunks > 1:
            dataloader = self._create_chunk_dataloader(
                ...
                functools.partial(self._collate, train=train),
            )
            return dataloader
    ...
    def _collate(self, batch: List[StrDict], train: bool = True) -> StrDict:
        (encoder_ids, encoder_mask, decoder_ids, decoder_mask, smiles) = self._transform_batch(batch, train)
...
class MoleculeListDataModule(BaseDataModule):
    """
    ...
    def __init__(
        ...
        masker: Union[TokensMasker, ReplaceTokensMasker] = None,
        **kwargs,
    ):
        super().__init__(**kwargs)
        if "mask" in task and masker is None:
    ...
    def _transform_batch(
        ...
        encoder_ids, encoder_mask = self._encoder(
            encoder_smiles, mask="mask" in self.task, add_sep_token=self.add_sep_token
        )
...
class ReactionListDataModule(BaseDataModule):
    """
    ...
    :param masker: if set, will be the masker to use on the encoder inputs.
    :param dataset_path: the path to the dataset on disc
    :param tokenizer: the tokenizer to use
    :param batch_size: the batch size to use
    :param max_seq_len: the maximum allowed sequence length
    ...
    def _get_sequences(self, batch: List[StrDict], train: bool) -> Tuple[List[str], List[str]]:
    ...
    def _transform_batch(
        ...
        reactants_smiles, product_smiles = self._get_sequences(batch, train)
        reactants_ids, reactants_mask = self._encoder(
            reactants_smiles, add_sep_token=self.add_sep_token and not self.reverse, mask=not self.reverse
        )
        product_ids, product_mask = self._encoder(
            product_smiles, add_sep_token=self.add_sep_token and self.reverse, mask=self.reverse
        )
        if not self.reverse:
            return reactants_ids, reactants_mask, product_ids, product_mask, reactants_smiles, product_smiles
        return product_ids, product_mask, reactants_ids, reactants_mask, product_smiles, reactants_smiles
