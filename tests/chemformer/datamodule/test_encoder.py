...
from aizynthmodels.chemformer.data.tokenizer import ReplaceTokensMasker
...
def test_encoder_add_sep_token(setup_tokenizer):
    smiles_list = ["CC(=O)O", "O"]
    tokenizer = setup_tokenizer()
    encoder = BatchEncoder(tokenizer, masker=False, max_seq_len=10000)

    id_tensor, mask_tensor = encoder(smiles_list, add_sep_token=True)

    assert tuple(id_tensor.shape) == (10, 2)
    assert tuple(mask_tensor.shape) == (10, 2)

    assert id_tensor[:, 0].tolist()[1:-2] == [tokenizer[token] for token in smiles_list[0]]
    assert id_tensor[0, 0] == tokenizer[tokenizer.special_tokens["start"]]
    assert id_tensor[-2, 0] == tokenizer[tokenizer.special_tokens["end"]]

    assert id_tensor[1, 1] == tokenizer["O"]
    assert id_tensor[0, 1] == tokenizer[tokenizer.special_tokens["start"]]
    assert id_tensor[2, 1] == tokenizer[tokenizer.special_tokens["end"]]
    assert id_tensor[3:-1, 1].tolist() == [tokenizer[tokenizer.special_tokens["pad"]]] * 6

    assert mask_tensor[:, 0].sum() == 0
    assert mask_tensor[:, 1].tolist() == [0, 0, 0] + [1] * 6 + [0]
